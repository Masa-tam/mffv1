...
TEST(DecoderTest, InspectFrameDimensionFailurePreservesOutputInfo)
{
    const auto result = mffv1::create_decoder({});
    ASSERT_TRUE(result.status.ok());
    ASSERT_NE(result.decoder, nullptr);
...
TEST(DecoderTest, DecodesCrcVerifiedMultiSliceRangeFrameThroughPublicApi)
{
    ...
    EXPECT_EQ(info.planes[0].width, stream.width);
    EXPECT_EQ(info.planes[0].height, stream.height);
    EXPECT_EQ(info.planes[0].stride_bytes, 2);
...
TEST(DecoderTest, FailedSliceDecodePreservesReferenceState)
{
    mffv1::DecoderOptions options;
    options.frame_width = 1;
    options.frame_height = 1;
    ...
    const std::array<std::byte, 1> truncated_keyframe{std::byte{0xff}};
    const auto failed_status =
        result.decoder->decode_frame(truncated_keyframe, output);
    ASSERT_FALSE(failed_status.ok());
    EXPECT_EQ(failed_status.code, mffv1::ErrorCode::SyntaxError);
    EXPECT_TRUE(failed_status.location.has_byte_offset);
    EXPECT_EQ(failed_status.location.byte_offset, 0u);
    EXPECT_EQ(storage[0], 0xdd);
