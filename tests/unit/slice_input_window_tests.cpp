...
TEST(SliceInputWindowTest, MapsSixteenBitInputStride)
{
    mffv1::syntax::StreamParameters stream;
    stream.width = 5;
    stream.height = 3;
    ...
    const std::array<std::uint16_t, 18> storage{
        0, 1, 2, 3, 4, 1000,
        5, 6, 7, 8, 9, 1001,
        10, 11, 12, 13, 14, 1002,
    };
    const mffv1::PlaneView plane{
        storage.data(),
        {mffv1::PlaneRole::Y, mffv1::SampleFormat::UInt16, 5, 3, 12},
    };
    const mffv1::FrameView frame{&plane, 1};
    mffv1::syntax::SliceDescriptor slice;
    slice.x = 1;
...
TEST(SliceInputWindowTest, RejectsZeroRasterDimensions)
{
    const auto stream = make_stream();
    ...
    EXPECT_FALSE(status.ok());
    EXPECT_EQ(status.code, mffv1::ErrorCode::InvalidArgument);
    EXPECT_EQ(status.message, "input slice dimensions must be non-zero");
}

TEST(SliceInputWindowTest, RejectsNullPlaneData)
{
    const auto stream = make_stream();
    std::array<std::uint8_t, 6> cb{};
    std::array<std::uint8_t, 6> cr{};
    ...
    EXPECT_FALSE(status.ok());
    EXPECT_EQ(status.code, mffv1::ErrorCode::InvalidArgument);
    EXPECT_EQ(status.message, "input slice plane does not match the stream");
